...
function uploadplay() {
    ...
    var diff = null;
    var obj = document.getElementsByName("diff2");
    for (var i in obj) if (obj[i].checked) diff = parseInt(obj[i].value);
    var mis, far, mxp, isScoreOnly;
    if (slst.songs.find((x) => {
        return x.title_localized.en == title
    }) == undefined) {
    ...
    if (lstInfo.wikiOnly || !sdb[songid]?.[diff] || sdb[songid][diff].note <= 0 || sdb[songid][diff].constant < 0) {
        var str = '没有 "' + title + '" 的 ' + ['PST', 'PRS', 'FTR', 'BYD', 'ETR'][diff] + ' 难度的完整谱面信息。';
        alert(str);
        return;
    }
    if (!isNaN(parseInt(document.getElementById('scoreOnly').value))) {
        isScoreOnly = true;
        var score = parseInt(document.getElementById('scoreOnly').value);
        var note = sdb[songid][diff].note;
        mis = 0;
        far = 2 * note - Math.floor((score + 1) / (1e7 / note / 2));
        mxp = score - Math.floor((2 * note - far) * (1e7 / note / 2));
...
function delplay() {
    ...
    var songid = lstInfo.id;
    var diff = null;
    var obj = document.getElementsByName("diffc");
    for (var i in obj) if (obj[i].checked) diff = parseInt(obj[i].value);
    var a = -1, b = {};
    ...
    if (a == -1) {
        alert('没有 "' + title + '" 的 ' + ['PST', 'PRS', 'FTR', 'BYD', 'ETR'][diff] + ' 难度的游玩记录。');
        return;
    }
